...
pub const Manager = wayland.client.zwlr.DataControlManagerV1;
pub const Device = wayland.client.zwlr.DataControlDeviceV1;
pub const Source = wayland.client.zwlr.DataControlSourceV1;
pub const Offer = wayland.client.zwlr.DataControlOfferV1;
...
const State = struct {
    ...
    /// Best MIME type the pending offer has advertised so far. `best_score == 0`
    /// means nothing usable has been offered yet; otherwise `best_mime[0..len]`
    /// holds the chosen, NUL-terminated type.
    best_score: u8 = 0,
...
pub fn dataControlOfferListener(_: *Offer, ev: Offer.Event, _: *Parent) void {
    ...
    // Strict greater-than keeps the first-seen type within a tier, matching
    // wl-clipboard's "first match wins" behaviour.
